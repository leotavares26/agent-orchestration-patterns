...
- [`react_loop.py`](examples/react_loop.py) — Pattern #1, interleaving `Thought -> Action -> Observation` so each step is chosen after seeing the previous observation, under a step budget. Run it with `python examples/react_loop.py`.
...
- [`supervisor.py`](examples/supervisor.py) — Pattern #5, a coordinator that decomposes a goal, delegates each sub-task to a focused specialist under an explicit handoff contract, and integrates the structured results. Run it with `python examples/supervisor.py`.
...
- [`memory_augmented.py`](examples/memory_augmented.py) — Pattern #8, assembling each turn's context from a bounded recent-turn window plus the stored facts retrieved for the current query, so the agent stays coherent after old turns leave the window. Run it with `python examples/memory_augmented.py`.
