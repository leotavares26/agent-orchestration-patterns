...
| [`supervisor.py`](supervisor.py) | #5 Supervisor (multi-agent) | A coordinator decomposing a goal, delegating each sub-task to a focused specialist under an explicit handoff contract, and integrating the structured results into one answer. |
...
| [`memory_augmented.py`](memory_augmented.py) | #8 Memory-augmented agent | Assembling context per turn from a bounded window of recent turns plus the stored facts retrieved for the current query, so salient facts survive after the turn that taught them leaves the window. |
